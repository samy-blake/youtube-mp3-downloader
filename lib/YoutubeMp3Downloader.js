...
class YoutubeMp3Downloader extends EventEmitter {
  ...
  async performDownload(task, callback) {
    ...
    // videoProgress.on("progress", function (progress) {
    ...
    //   });
    // });
    // ytdl(videoUrl).pipe(videoProgress).pipe(videoReadStream);
    ytdl(videoUrl).pipe(videoReadStream);
